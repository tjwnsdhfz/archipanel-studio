  ...
  return (
    <main className="welcome">
      ...
      <section className="recent-panel">
        ...
        <div className="welcome-guide"><h2>처음이라면 이렇게 시작하세요</h2><ol><li>패널 크기를 정하고 PNG·JPG·WebP 이미지를 추가합니다.</li><li>도형·텍스트를 배치하고 작업 파일을 저장합니다.</li><li>2,400px PNG로 공유하거나 연결된 서버에서 인쇄용 PDF를 출력합니다.</li></ol><button disabled={demoBusy || STATIC_MODE} onClick={() => void openDemo()}>{demoBusy ? "예시 준비 중…" : "서버 연결 예시 열기"}</button><p>PNG·JPG·WebP는 각 25MB·4천만 픽셀, 작업 백업은 합계 100MB까지 지원합니다. 기본 작업은 브라우저에서 처리합니다. PDF·PSD 분석과 인쇄 출력은 서버 연결이 필요합니다.</p></div>
    ...
    <main className="studio">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button title="현재 보드를 긴 변 2,400px PNG로 저장합니다. 인쇄용 PDF는 서버가 필요합니다." onClick={() => void downloadCanvasPreview(project!.name).then(() => setNotice("2,400px PNG 다운로드를 요청했습니다. 인쇄 크기·DPI를 보장하는 출력은 아닙니다.")).catch(reason => setNotice(String(reason)))}>공유 PNG</button>
