...
import {
  ...
  Crop, Group, Ungroup, Upload, ZoomIn, ZoomOut, FlipHorizontal2, FlipVertical2, Grid3X3, RefreshCw,
} from "lucide-react";
  ...
  return (
    <main className="studio">
      ...
      <aside className="inspector">
        ...
        {tab === "assets" && <AssetsPanel onAdd={() => assetInput.current?.click()} onFont={() => fontInput.current?.click()} />}
...
function LayersPanel({ board }: { board: NonNullable<ReturnType<typeof useStudio.getState>["project"]>["boards"][number] }) {
  ...
  return <div className="panel-scroll"><Section title="ALIGN / 자동 정렬"><label><span>기준</span><select value={state.alignmentReference} onChange={(e) => state.setAlignmentReference(e.target.value as "selection")}><option value="selection">선택 경계</option><option value="board">보드</option><option value="safe">안전 여백</option><option value="key">핵심 객체</option></select></label><div className="alignment-grid"><button title="왼쪽" onClick={() => state.alignSelected("left")}><AlignLeft /></button><button title="가로 중앙" onClick={() => state.alignSelected("hcenter")}><AlignCenter /></button><button title="오른쪽" onClick={() => state.alignSelected("right")}><AlignRight /></button><button title="위" onClick={() => state.alignSelected("top")}><AlignStartVertical /></button><button title="세로 중앙" onClick={() => state.alignSelected("vcenter")}><AlignCenterVertical /></button><button title="아래" onClick={() => state.alignSelected("bottom")}><AlignEndVertical /></button></div><div className="alignment-grid"><button title="수평 동일 간격" onClick={() => state.distributeSelected("h-gap")}>H↔</button><button title="수직 동일 간격" onClick={() => state.distributeSelected("v-gap")}>V↕</button><button title="수평 중심 분배" onClick={() => state.distributeSelected("h-center")}>H●</button><button title="수직 중심 분배" onClick={() => state.distributeSelected("v-center")}>V●</button></div><div className="tidy-row"><label><span>거터 mm</span><input type="number" min="0" value={gap} onChange={(e) => setGap(Number(e.target.value))} /></label><button onClick={() => state.tidySelected(gap)}><Grid3X3 size={15} /> Tidy Grid</button></div></Section><div className="layer-actions"><button title="그룹" onClick={state.groupSelected}><Group size={15} /></button><button title="그룹 해제" onClick={state.ungroupSelected}><Ungroup size={15} /></button><button title="복제" onClick={state.duplicateSelected}><BoxSelect size={15} /></button><button title="삭제" onClick={state.deleteSelected}><Trash2 size={15} /></button></div><div className="layer-list">{[...elements].reverse().map((element) => { const active = element.type === "group" ? element.childIds.some((id) => state.selectedIds.includes(id)) : state.selectedIds.includes(element.id); return <button key={element.id} className={active ? "layer-row active" : "layer-row"} onClick={(event) => { const ids = element.type === "group" ? element.childIds : [element.id]; state.setSelection(event.shiftKey ? Array.from(new Set([...state.selectedIds, ...ids])) : ids); }}><span className="layer-type">{element.type === "text" ? "T" : element.type === "shape" ? "◇" : element.type === "pdf" ? "P" : element.type === "group" ? "G" : "I"}</span><span className="layer-name">{element.name}</span><span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); state.updateElement(element.id, { visible: !element.visible }); }}>{element.visible ? <Eye size={14} /> : <EyeOff size={14} />}</span><span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); state.updateElement(element.id, { locked: !element.locked }); }}>{element.locked ? <Lock size={13} /> : <LockOpen size={13} />}</span></button>; })}</div></div>;
}

function AssetsPanel({ onAdd, onFont }: { onAdd: () => void; onFont: () => void }) {
  const state = useStudio();
  return <div className="panel-scroll"><div className="asset-buttons"><button className="primary" onClick={onAdd}><Upload size={15} /> 자산 추가</button><button onClick={onFont}><TextCursorInput size={15} /> 글꼴 추가</button></div><div className="asset-list">{state.project?.assets.map((asset) => <div className="asset-row" key={asset.id}><div className="asset-icon">{asset.mime === "application/pdf" ? "PDF" : "IMG"}</div><div><strong>{asset.name}</strong><small>{asset.widthPx && asset.heightPx ? `${asset.widthPx} × ${asset.heightPx}px` : asset.pageCount ? `${asset.pageCount} pages` : `${Math.round(asset.sizeBytes / 1024)} KB`}</small></div></div>)}</div>{state.project?.fonts.length ? <Section title="PROJECT FONTS">{state.project.fonts.map((font) => <div className="font-row" key={font.id}><span style={{ fontFamily: font.family }}>{font.family}</span><small>{font.embeddingAllowed === "unknown" ? "권한 확인 필요" : "임베딩 허용"}</small></div>)}</Section> : null}</div>;
}
