# ArchiPanel Studio 1.5.0

Studio 1.5는 기존 mm 기반 패널 편집·PSD/PSB 연결·설계설명서 경로를 유지하면서, 설명 가능한 디자인 크리틱과 명시적 로컬 취향 학습을 추가합니다.

- `크리틱`: 원본·흑백·축소판·블러 보기, 3단계 시각 위계, 40×40 밀도/백색 공간, 예상 시선 흐름
- 7개 설명 가능한 평가축과 문제 요소 ID, 이유, 수정 후보를 함께 표시하며 패널을 자동 수정하지 않음
- Narrative/Hero/Technical 3안을 같은 진단축으로 비교하고 선택 이유를 기록
- 사용자가 `이 수정에서 학습`을 누른 경우에만 7개 익명 특징 가중치를 이 브라우저의 IndexedDB에 저장
- 공개 샘플: `https://archipanel-studio-mvp.netlify.app`에서 서버 없이 4개 진단 보기와 규칙 기반 점수 시연
...
## Studio 1.5 디자인 크리틱 흐름

`크리틱 ON → 원본/흑백/축소판/블러 비교 → 위계·밀도·예상 시선 확인 → 3안 비교 → 선택 이유 기록 → 수동 편집 → 이 수정에서 학습` 순서로 사용합니다.

- 편집이 끝난 뒤 500ms 후 현재 보드 revision hash로 진단을 갱신하고 동일 revision 결과는 IndexedDB에서 재사용합니다.
- 숨김 요소는 분석에서 제외하고 잠금 요소는 분석에 포함하되 자동 수정 대상으로 삼지 않습니다.
- 백색 공간은 그 자체로 감점하지 않습니다. 연속성, 과밀, 밀도 균형, 거터, 안전 여백과 격자 정렬을 분리해 봅니다.
- 예상 시선 흐름은 승인된 콘텐츠 읽기 순서를 우선 사용합니다. 읽기 순서가 없으면 낮은 신뢰도의 좌상단 기반 임시 경로임을 표시합니다.
- 취향 특징은 `위계 분리 / 주초점 비율 / 백색 공간 / 밀도 균형 / 격자 정렬 / 간격 리듬 / 읽기 흐름` 7개만 저장합니다. 이미지, 문장, 파일 경로는 저장하지 않습니다.
- 20회 이전에는 `학습 중`으로 표시하며 언제든 전체 취향 프로필을 초기화할 수 있습니다.
- 진단은 미학적 정답이나 실제 아이트래킹이 아닙니다. 설계 의도에 따라 경고를 무시할 수 있습니다.
